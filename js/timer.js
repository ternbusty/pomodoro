...
window.sync = function () {
  ...
  fetch("http://127.0.0.1:8000/status/", {
    ...
    .then((data) => {
      ...
      if (backend_status === "finished") {
        if (window.timer_status !== "finished") summarize(true);
      } else if (backend_status === "not_started");
      else if (backend_status === "work") {
        break2Work(true);
      } else if (backend_status === "break") {
        work2break(true);
      }
...
function setTaskStr(task_name) {
  if (task_name[0] === '"') task_name = task_name.slice(1, 0);
  if (task_name[-1] === '"') task_name = task_name.slice(0, -1);
...
function work2break(isCalledBackend = false, add = 0) {
  ...
  // Call backend
  if (!isCalledBackend) {
    let url = "http://127.0.0.1:8000/start_break/";
...
function break2Work(isCalledBackend = false) {
  ...
  // Call backend
  if (!isCalledBackend) {
    let url = "http://127.0.0.1:8000/start_work/";
  ...
  // tb.addRow(window.history_table, [dt.createDateStr(), `Start ${task_str}`]);
  // Update elements
  window.task_input.disabled = true;
...
window.start = function (isCalledBackend = false) {
  // Call backend
  if (!isCalledBackend) {
    fetch("http://127.0.0.1:8000/terminate/", {
      method: "PUT",
    });
